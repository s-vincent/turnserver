...
/**
 * \file test_client_tls_tcp.c
 * \brief TLS TURN-TCP client example.
...
int main(int argc, char** argv)
{
  ...
  nb = turn_parse_message(buf2, nb, &message, tabu, &tabu_size);
  printf("parsing done\n");
...
*/
  ...
  if(turn_parse_message(buf2, nb, &message, tabu, &tabu_size) == -1)
  {
  ...
  printf("Receive ConnectionAttempt!\n");

  if(nb > 0)
  {
    turn_parse_message(buf2, nb, &message, tabu, &tabu_size);
